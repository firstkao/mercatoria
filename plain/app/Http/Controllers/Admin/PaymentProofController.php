...
class PaymentProofController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $proofs = DB::table('payment_proofs')
            ...
            ->select(
                ...
                'users.full_name',
                'users.email',
                'payment_methods.label as payment_method_label'
            )
    ...
    public function approve(int $proof): RedirectResponse
    {
        DB::transaction(function () use ($proof): void {
            ...
            $order = DB::table('orders')->where('id', $record->order_id)->lockForUpdate()->firstOrFail();
