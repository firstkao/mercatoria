-- Manual SQL for Mercatoria payment-review support.
-- Run this in the same database where the app runs.
-- This file is intentionally safe to re-run: it checks for existing tables/columns.
...
ALTER TABLE payment_proofs
    ADD COLUMN IF NOT EXISTS payment_stage VARCHAR(10) NOT NULL DEFAULT 'dp' AFTER payment_method_id;

ALTER TABLE payment_proofs
    ADD INDEX IF NOT EXISTS payment_proofs_order_status_index (order_id, status);
